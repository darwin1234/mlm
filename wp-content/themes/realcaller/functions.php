...
if ( ! function_exists( 'realcaller_setup_theme' ) ) {
	/**
	 ...
	 * @return void
	 */
	function realcaller_setup_theme() {
	...
	add_action( 'after_setup_theme', 'realcaller_setup_theme' );
	 ...
	 * @return void
	 */
	function realcaller_load_editor_styles() {
		if ( is_admin() ) {
			wp_enqueue_style( 'editor-style', get_theme_file_uri( 'style-editor.css' ) );
		}
	}
	add_action( 'enqueue_block_assets', 'realcaller_load_editor_styles' );
...
if ( ! function_exists( 'realcaller_add_user_fields' ) ) {
	/**
	 ...
	 * @return array
	 */
	function realcaller_add_user_fields( $fields ) {
		// Add new fields.
	...
	add_filter( 'user_contactmethods', 'realcaller_add_user_fields' );
}
 ...
 * @return bool
 */
function realcaller_filter_media_comment_status( $open, $post_id = null ) {
...
add_filter( 'comments_open', 'realcaller_filter_media_comment_status', 10, 2 );
 ...
 * @return string
 */
function realcaller_custom_edit_post_link( $link ) {
	return str_replace( 'class="post-edit-link"', 'class="post-edit-link badge bg-secondary"', $link );
}
add_filter( 'edit_post_link', 'realcaller_custom_edit_post_link' );
 ...
 */
function realcaller_custom_edit_comment_link( $link ) {
	return str_replace( 'class="comment-edit-link"', 'class="comment-edit-link badge bg-secondary"', $link );
}
add_filter( 'edit_comment_link', 'realcaller_custom_edit_comment_link' );
 ...
 * @return string
 */
function realcaller_oembed_filter( $html ) {
	return '<div class="ratio ratio-16x9">' . $html . '</div>';
}
add_filter( 'embed_oembed_html', 'realcaller_oembed_filter', 10 );
 ...
 * @return void
 */
function realcaller_widgets_init() {
	// Area 1.
	register_sidebar(
		array(
...
add_action( 'widgets_init', 'realcaller_widgets_init' );
 ...
 * @return string
 */
function realcaller_password_form() {
	global $post;
...
add_filter( 'the_password_form', 'realcaller_password_form' );
...
if ( ! function_exists( 'realcaller_comment' ) ) {
	 ...
	 * @return array
	 */
	function realcaller_custom_commentform( $args = array(), $post_id = null ) {
		if ( null === $post_id ) {
			$post_id = get_the_ID();
		}
	...
	add_filter( 'comment_form_defaults', 'realcaller_custom_commentform' );
}
 ...
 * @return void
 */
function realcaller_scripts_loader() {
...
add_action( 'wp_enqueue_scripts', 'realcaller_scripts_loader' );


// Restrict WooCommerce cart to a specific product ID
function restrict_cart_to_single_specific_product() {
...
add_action('woocommerce_before_cart', 'restrict_cart_to_single_specific_product');
...
//add_action('template_redirect', 'redirect_to_checkout_if_single_specific_product');

// Prevent checkout if the cart contains any product other than the specified product
function check_cart_contents_before_checkout_specific_product() {
    // Define the product ID you want to restrict
    $specific_product_id = 11; // Change this to your desired product ID
...
add_action('woocommerce_checkout_process', 'check_cart_contents_before_checkout_specific_product');


//add_action('template_redirect','check_if_logged_in');
function check_if_logged_in()
{
	$woocommerce_checkout_page_id = 8;
...
add_action('init', 'register_dealer');

function register_dealer() {
	...
	if(!is_user_logged_in()){
		if (isset($_POST['register_dealer'])) 
		{
			...
			$bmlm_sponsor_id =   generate_random_string(10);
			...
			$order->save();
			   ...
			   // Optionally, set the payment method and mark the order as paid
			$order->set_payment_method('bacs'); // Replace with the desired payment method
...
add_action('init', 'register_as_client');

function register_as_client() 
{
    if (!is_user_logged_in()) 
    {
