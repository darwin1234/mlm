...
if ( ! class_exists( 'WKWP_Wallet_WK_Caching_Core' ) ) {
	...
	class WKWP_Wallet_WK_Caching_Core {
		...
		public static function register() {
			$configuration = array(
				'basename'    => BMLM_PLUGIN_BASENAME,
				'version'     => self::$version,
				'plugin_path' => BMLM_PLUGIN_FILE,
				'class'       => __CLASS__,
			);
		...
		public static function load_files() {
			$get_global_path = BMLM_PLUGIN_FILE . 'wk_caching/';

			if ( false === @file_exists( $get_global_path . '/includes/class-wk-caching.php' ) ) {
				_doing_it_wrong( __FUNCTION__, esc_html__( 'WK Caching Core should be present in folder \'wk_caching\includes\' in order to run this properly.', 'binary-mlm' ), esc_html( self::$version ) );
			...
			if ( WK_CACHING_VERSION === self::$version ) {
				...
				_doing_it_wrong( __FUNCTION__, esc_html__( 'WK Caching Core should be at the same version as declared in your "class-wk-caching-core-loader.php"', 'binary-mlm' ), esc_html( self::$version ) );
				if ( defined( 'WKWC_DEV' ) && true === WKWC_DEV ) {
					die( 0 );
				}
				return false;
