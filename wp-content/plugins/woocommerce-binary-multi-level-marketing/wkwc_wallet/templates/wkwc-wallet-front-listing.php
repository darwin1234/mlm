...
<div class="wkwc-wallet-front-container">
	<div class="add-wallet-wrapper">
		<div class="amount-container">
			<div class="amount-heading <?php echo esc_attr( $width_class ); ?>">
				<h4><?php esc_html_e( 'Wallet Amount', 'wkwc_wallet' ); ?></h4>
				<span class="wallet-money-style"><?php echo esc_html( get_woocommerce_currency_symbol() ); ?><?php echo esc_html( wc_format_decimal( $wallet_amount, 2 ) ); ?>
					<a href="<?php echo esc_url( wc_get_endpoint_url( 'wkwc_wallet/transfer' ) ); ?>" class="page-title-action wallet-transfer-link wallet-transfer"><?php esc_html_e( 'Wallet Transfer ', 'wkwc_wallet' ); ?></a>
				</span>
			</div>
			<?php
			if ( 'sms' === $otp_method ) {
				...
				<div class="twilio-sms-mobile <?php echo esc_attr( $width_class ); ?>"">
					<span class="twilio-sms-mobile-title"><?php esc_html_e( 'Twilio SMS Number (with country code)', 'wkwc_wallet' ); ?></span>
					<div class="sms-number-wrap">
						<?php
						if ( ! empty( $phone ) ) {
							?>
							<span id="wkwc_wallet_twilio_phone"><?php echo esc_html( $phone ); ?></span>
							<span id="wkwc_wallet_twilio_edit" title="<?php esc_attr_e( 'Edit', 'wkwc_wallet' ); ?>" class="wkwc-wallet-twilio-action dashicons dashicons-edit"></span>
							<?php
						} else {
							?>
							<input value="" id="wkwc_wallet_twilio_sms_number" type="text" placeholder="<?php esc_attr_e( 'Enter your mobile number...', 'wkwc_wallet' ); ?>">
							<span id="wkwc_wallet_twilio_update" title="<?php esc_attr_e( 'Update', 'wkwc_wallet' ); ?>" class="wkwc-wallet-twilio-action dashicons dashicons-update"></span>
							<?php
						}
						?>
			...
			<div class="process-money <?php echo esc_attr( $width_class ); ?>"">
				...
				<input type="submit" value="<?php esc_attr_e( 'Add to Wallet', 'wkwc_wallet' ); ?>" class="add_wallet_money_button wallet-button small" name="wkwc_wallet_add_money" />
				</form>
			</div>
		</div>
	</div>
	...
	<div class="wallet-transactions-wrapper">
		<h4><?php esc_html_e( 'Wallet Transactions', 'wkwc_wallet' ); ?></h4>
			...
			<div class="action-wrapper">
				<div class="form-row wkwc-wallet-bulk-wrap">
					<select name="wallet_action" id="wkwc_wallet_action">
						<option value=""><?php esc_html_e( 'Bulk Actions', 'wkwc_wallet' ); ?></option>
						<option value="delete"><?php esc_html_e( 'Delete', 'wkwc_wallet' ); ?></option>
					</select>
					<input type="button" class="front_bulk_action" id="wkwc_wallet_bulk_delete" value = "<?php esc_attr_e( 'Apply', 'wkwc_wallet' ); ?>" />
				...
				<div class="wkwc-wallet-total-transaction-count">
					<span><?php echo wp_sprintf( /* Translations: %s: Transaction count. */ esc_html__( '%s Transactions', 'wkwc_wallet' ), esc_html( $transaction_count ) ); ?></span>
				</div>
				<?php } ?>
			</div>
		</div>
				...
				<table class="woocommerce-orders-table woocommerce-MyAccount-orders shop_table shop_table_responsive my_account_orders account-orders-table">
					<thead>
						<tr>
							...
							<th><?php esc_html_e( 'ID', 'wkwc_wallet' ); ?></th>
							<th><?php esc_html_e( 'Amount', 'wkwc_wallet' ); ?></th>
							<th><?php esc_html_e( 'Type', 'wkwc_wallet' ); ?></th>
							<th><?php esc_html_e( 'Date', 'wkwc_wallet' ); ?></th>
						</tr>
					</thead>
					...
					$transfer_transaction = array(
						'to'   => __( 'Transfer to ', 'wkwc_wallet' ),
						'from' => __( 'Received from ', 'wkwc_wallet' ),
					);
			...
			<?php else : ?>
				<div class="woocommerce-message woocommerce-message--info woocommerce-Message woocommerce-Message--info woocommerce-info">
					<a class="woocommerce-Button button" href="<?php echo esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>">
						<?php esc_html_e( 'Go shop', 'wkwc_wallet' ); ?>
					</a>
					<?php esc_html_e( 'No order has been made yet.', 'wkwc_wallet' ); ?>
				</div>
			<?php endif; ?>
	</div>
		<?php
		...
		<div class="woocommerce-pagination woocommerce-pagination--without-numbers woocommerce-Pagination wallet-pagination" style="margin-top:10px;">
				...
				<a class="woocommerce-button woocommerce-button--previous woocommerce-Button woocommerce-Button--previous button" href="<?php echo esc_url( wc_get_endpoint_url( 'wkwc_wallet', $wc_paged - 1 ) ); ?>"><?php esc_html_e( 'Previous', 'wkwc_wallet' ); ?></a>
			<?php endif; ?>

				<?php if ( ceil( count( $all_transactions ) / $limit ) > $wc_paged ) : ?>
				<a class="woocommerce-button woocommerce-button--next woocommerce-Button woocommerce-Button--next button" href="<?php echo esc_url( wc_get_endpoint_url( 'wkwc_wallet', $wc_paged + 1 ) ); ?>"><?php esc_html_e( 'Next', 'wkwc_wallet' ); ?></a>
			<?php endif; ?>
