...
if ( ! trait_exists( 'WC_Email_WKWC_Wallet_Trait' ) ) {
	...
	trait WC_Email_WKWC_Wallet_Trait {
		...
		public function get_default_additional_content() {
			return __( 'Thank you for choosing {site_title}!', 'wkwc_wallet' );
		}
		...
		public function wkwc_wallet_get_common_mail_data( $mail_data ) {
			$name = empty( $mail_data['name'] ) ? __( 'Someone', 'wkwc_wallet' ) : $mail_data['name'];
			return array(
				'hi_msg'      => html_entity_decode( wp_sprintf( /* translators: %s: Customer name. */ esc_html__( 'Hi %s,', 'wkwc_wallet' ), esc_html( $name ) ), ENT_QUOTES, 'UTF-8' ),
				'message'     => empty( $mail_data['message'] ) ? __( 'Message from Admin', 'wkwc_wallet' ) : $mail_data['message'],
