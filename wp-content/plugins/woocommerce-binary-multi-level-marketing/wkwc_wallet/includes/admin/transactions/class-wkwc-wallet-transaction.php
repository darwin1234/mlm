...
if ( ! class_exists( 'WKWC_Wallet_Transaction' ) ) {
	...
	class WKWC_Wallet_Transaction {
		...
		public function wkwc_wallet_transaction_view( $id ) {
			...
			?>
			<div class="wrap">
				<h1 class="wp-heading-inline"><?php esc_html_e( 'Wallet Transaction Details', 'wkwc_wallet' ); ?></h1>
					...
					<table class="wallet-transaction-view">
						<tbody>
							...
							if ( $sender_id > 0 || $customer_id > 0 || $order_id > 0 ) {
								?>
							<tr>
								<th><?php esc_html_e( 'Amount', 'wkwc_wallet' ); ?></th>
								<td><?php echo wc_price( $amount ); ?></td>
							</tr>
								<?php
								if ( $order_id > 0 ) {
									$order_url = WKWC_Addons::get_order_edit_url( $order_id );
									...
									?>
							<tr>
								<th><?php esc_html_e( 'Order ID', 'wkwc_wallet' ); ?></th>
								<td><?php echo '<a href="' . esc_url( $order_url ) . '" >#' . esc_html( $order_id ) . '</a>'; ?></td>
							</tr>
								<?php } ?>
							<tr>
								<th><?php esc_html_e( 'Action', 'wkwc_wallet' ); ?></th>
								...
								if ( $customer_id > 0 ) {
									...
									?>
								<tr>
									<th><?php esc_html_e( 'Receiver', 'wkwc_wallet' ); ?></th>
								...
								if ( $sender_id > 0 ) {
									...
									?>
								<tr>
									<th><?php esc_html_e( 'Payer', 'wkwc_wallet' ); ?></th>
								...
								?>
							<tr>
								<th><?php esc_html_e( 'Transaction On', 'wkwc_wallet' ); ?></th>
								<td><?php echo gmdate( 'M d, Y g:i:s A', strtotime( $transaction_date ) ); ?></td>
							</tr>

							<tr>
								<th><?php esc_html_e( 'Transaction Note', 'wkwc_wallet' ); ?></th>
								...
								?>
								<tr>
									<td colspan="2"><?php esc_html_e( 'Not a valid transaction.', 'wkwc_wallet' ); ?></td>
								</tr>
								<?php
							}
							?>
				...
				<div class="main-container">
					...
					<div style="padding: 6px; background-color: #f8f8f8;border: solid 1px #ccc;border-radius: 3px; display:inline-block; width:100%">
						<div style="float: left; margin-left: 3px;">
							...
							<h4 style="margin:0; padding:0; font-size:17px; font-weight:500;"><?php esc_html_e( 'Wallet Amount', 'wkwc_wallet' ); ?></h4>
						</div>
					...
					<div class = "form-wrapper wallet-transfer-container">
						<div>
							<table id="wkwc_wallet_transfer_from">
								<tr>
									<td>
										<label for="wkwc_wallet_receiver"><?php esc_html_e( 'Receiver&#8217;s Email : ', 'wkwc_wallet' ); ?></label>
									</td>
									<td>
										<input value="<?php echo esc_attr( $receiver ); ?>" type="email" id="wkwc_wallet_receiver" placeholder="<?php esc_attr_e( 'e.g. example@xyz.com', 'wkwc_wallet' ); ?>" />
									</td>
								</tr>
								<tr>
									<td>
										<label for="wkwc_wallet_pay_amount"><?php esc_html_e( 'Amount To Transfer: ', 'wkwc_wallet' ); ?></label>
									</td>
									<td>
								...
								<tr class="wkwc_wallet_transaction_note">
									<td>
										<label for="wkwc_wallet_pay_note"><?php esc_html_e( 'Transaction Note: ', 'wkwc_wallet' ); ?></label>
									</td>
									<td>
								...
								<tr>
									<td>
										...
										<button type="buttun" id="wkwc_wallet_transfer_money" class="wallet-button"><?php esc_html_e( 'Transfer', 'wkwc_wallet' ); ?></button>
										<button type="buttun" id="wkwc_wallet_verify_otp" class="wallet-button wallet-verify wkwc-wallet-hide"><?php esc_html_e( 'Verify & Transfer', 'wkwc_wallet' ); ?></button>
										<button type="buttun" id="wkwc_wallet_resend_otp" class="wallet-button wallet-resend wkwc-wallet-hide"><?php esc_html_e( 'Resend', 'wkwc_wallet' ); ?></button>
		...
		public function wkwc_wallet_verify_transfer( $posted_data ) {
			...
			$result = array(
				'success' => false,
				'message' => esc_html__( 'OTP could not be sent', 'wkwc_wallet' ),
			);

			if ( empty( $receiver ) ) {
				$result['message'] = __( 'Customer Email is required.', 'wkwc_wallet' );
				return $result;
			}

			if ( empty( $amount ) || ! is_numeric( $amount ) || $amount <= 0 ) {
				$result['message'] = __( 'Please enter a numeric amount greater than 0.', 'wkwc_wallet' );
				return $result;
			}
			...
			if ( ! empty( $max_transfer ) && $amount > $max_transfer ) {
				$result['message'] = wp_sprintf( /* translators: %1$s: Max transfer value. */ esc_html__( 'You can not transfer more than %s', 'wkwc_wallet' ), get_woocommerce_currency_symbol() . ' ' . number_format( $max_transfer, 2, wc_get_price_decimal_separator(), wc_get_price_thousand_separator() ) );
				return $result;
			}
			...
			if ( ! $receiver_user instanceof \WP_User ) {
				$result['message'] = __( 'Receiver does not exist.', 'wkwc_wallet' );
				return $result;
			}
			...
			if ( $receiver === $sender_email ) {
				$result['message'] = __( 'Cannot Transfer money to yourself.', 'wkwc_wallet' );
				return $result;
			}
			...
			if ( floatval( $wallet_balance ) < floatval( $amount ) ) {
				$result['message'] = __( 'You have insufficient wallet balance.', 'wkwc_wallet' );
				return $result;
			}
			...
			$otp_data      = array(
				...
				'amount'       => $amount,
				'note'         => $note,
				'message'      => wp_sprintf( /* translators: %1$s: Blog info, %2$s: Transfer amount, %3$s: Receiver name. */ esc_html__( 'You have initiated a transfer of amount: %1$s to %2$s on the site [%3$s]', 'wkwc_wallet' ), get_woocommerce_currency_symbol() . ' ' . number_format( $amount, 2, wc_get_price_decimal_separator(), wc_get_price_thousand_separator() ), $receiver_name, get_bloginfo( 'name' ) ),
			);

			if ( $otp_enabled ) {
				$result = $this->wkwc_wallet_send_otp( $otp_data );
			} else {
				$transferred = $this->wkwc_wallet_transfer_money( $otp_data );

				if ( $transferred ) {
					$result['success'] = 'transfered';
		...
		public function wkwc_wallet_send_otp( $otp_data ) {
			...
			$otp_limit      = empty( $wallet_setting['otp_limit'] ) ? 2 : $wallet_setting['otp_limit'];
			...
			$result = array(
				'success'     => false,
				'otp_seconds' => $otp_sec,
				'message'     => esc_html__( 'Unable to sent the OTP.', 'wkwc_wallet' ),
			);

			$random_code = random_int( 100000, 999999 );
			$message    .= wp_sprintf( /* translators: %s: OTP */ esc_html__( ' Your six digit OTP for this wallet transaction is: %s', 'wkwc_wallet' ), $random_code );
			...
			if ( 'mail' === $otp_method ) {
				...
				$success_message = wp_sprintf( /* translators: %s: OTP */ esc_html__( 'Your six digit OTP for wallet transaction has been successfully sent to the email: %s', 'wkwc_wallet' ), $masked_email );
				...
				if ( empty( $sender_phone ) ) {
					$result['message'] = esc_html__( 'You must update your phone number along with country from my-account/wkwc_wallet section.', 'wkwc_wallet' );
				} else {
		 ...
		 * @param bool  $unset Unset wallet payment.
		 *
		 * @return bool
		 */
		public function wkwc_wallet_set_wallet_payment( $wallet_setting = array(), $unset = false ) {
			$method_set = false;
