...
if ( ! class_exists( 'WKWC_Wallet_Customer' ) ) {
	...
	class WKWC_Wallet_Customer {
		 ...
		 * @return void
		 */
		public function wkwc_show_customer_wallet_table( $setting_page = '' ) {
			?>
			<div class="wrap">
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=' . $setting_page . '&action=update' ) ); ?>" class='page-title-action'><?php esc_html_e( 'Manual Transaction', 'wkwc_wallet' ); ?></a>
				<form method="post" style="display: inline-block;"><input type="submit" name="wkwc_wallet_export_csv" class="page-title-action export_wallet_details_csv" value="<?php esc_attr_e( 'Export Wallet', 'wkwc_wallet' ); ?>">
					<?php wp_nonce_field( 'wkwc_wallet_cashback_product', 'wkwc_wallet_cashback_product_nonce' ); ?>
				</form>
				<?php
				...
				$transaction = filter_input( INPUT_GET, 'transaction', FILTER_SANITIZE_FULL_SPECIAL_CHARS );

				if ( in_array( $transaction, array( 'debit', 'credit' ), true ) ) {
					$msg = esc_html__( 'Amount is debited successfully.', 'wkwc_wallet' );

					if ( 'credit' === $transaction ) {
						$msg = esc_html__( 'Amount is credited successfully.', 'wkwc_wallet' );
					}
					?>
