...
if ( ! class_exists( 'WKWC_Wallet_Table' ) ) {
	...
	class WKWC_Wallet_Table extends WP_List_Table {
		...
		public function __construct() {
			parent::__construct(
				array(
					'singular' => esc_html__( 'Customer Wallet', 'wkwc_wallet' ),
					'plural'   => esc_html__( 'Customer Wallet', 'wkwc_wallet' ),
					'ajax'     => false,
				)
			);
		}
		...
		public function get_columns() {
			$columns = array(
				'id'           => __( 'ID', 'wkwc_wallet' ),
				'username'     => __( 'Username', 'wkwc_wallet' ),
				'email'        => __( 'Email', 'wkwc_wallet' ),
				'location'     => __( 'Location', 'wkwc_wallet' ),
				'orders'       => __( 'Orders', 'wkwc_wallet' ),
				'wallet_money' => __( 'Wallet Money', 'wkwc_wallet' ),
				'last_order'   => __( 'Last Order', 'wkwc_wallet' ),
				'user_actions' => __( 'Actions', 'wkwc_wallet' ),
			);
				...
				case 'user_actions':
					...
					if ( $orders ) {
						$actions['link'] = array(
							...
							'name'   => __( 'Link prev orders', 'wkwc_wallet' ),
							'action' => 'link',
						);
					}
		...
		private function table_data( $search, $perpage = -1 ) {
			...
			foreach ( $items as $item ) {
				...
				if ( $orders ) {
					$actions['link'] = array(
						'url'    => wp_nonce_url( add_query_arg( 'link_orders', $user->ID ), 'link_orders' ),
						'name'   => esc_html__( 'Link previous orders', 'wkwc_wallet' ),
						'action' => 'link',
					);
				}
				...
				if ( ! empty( $last_order ) ) {
					...
					$last_order = '<a href="' . admin_url( 'post.php?post=' . $last_order->get_id() . '&action=edit' ) . '">' . _x( '#', 'hash before order number', 'wkwc_wallet' ) . $last_order->get_order_number() . '</a> &ndash; ' . date_i18n( get_option( 'date_format' ), strtotime( $last_order->get_date_created() ) );
				} else {
					$last_order = '-';
				}
