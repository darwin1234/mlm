...
class WKWC_Wallet_SMS_Handler {
	...
	public function wkwc_wallet_send_otp_sms( $phone_number, $message, $result = array(), $otp = '' ) {
		$result['message'] = esc_html__( 'There are some issue in sending SMS, contact the administrator.', 'wkwc_wallet' );
		...
		if ( ! empty( $phone_number ) ) {
			try {
				...
				$sid            = empty( $wallet_setting['twilio_sid'] ) ? '' : $wallet_setting['twilio_sid'];
				$token          = empty( $wallet_setting['auth_token'] ) ? '' : $wallet_setting['auth_token'];
				$twilio_number  = empty( $wallet_setting['twilio_number'] ) ? '' : $wallet_setting['twilio_number'];
				...
				$success_message = wp_sprintf( /* translators: %s: OTP */ esc_html__( 'Your six digit OTP for wallet transaction has been successfully sent to the phone: %s', 'wkwc_wallet' ), $masked_phone );
				...
				$result['message'] = __( 'SMS Service issue, Try again later.', 'wkwc_wallet' );
				$code              = $e->getCode();
