...
if ( ! class_exists( 'WKWC_Wallet_Front_Functions' ) ) {
	...
	class WKWC_Wallet_Front_Functions {
		...
		public function wkwc_wallet_public_scripts() {
			...
			wp_enqueue_script( 'wkwc-wallet-front-script', WKWC_WALLET_SUBMODULE_URL . 'assets/wkwc-wallet-front.js', array(), WKWC_WALLET_SCRIPT_VERSION, true );
			...
			if ( false !== strpos( $wp_query->query_vars['wkwc_wallet'], 'view' ) ) {
				...
				if ( isset( $_REQUEST['wkwc_wallet_add_money'] ) ) {
					...
					if ( ! isset( $posted_data['add_wallet_money'] ) || ! wp_verify_nonce( $posted_data['wkwc_wallet_add_amount_nonce_action'], 'wkwc_wallet_add_amount_nonce' ) ) {
						wc_print_notice( esc_html__( 'Sorry, your nonce did not verify.', 'wkwc_wallet' ), 'error' );
					} else {
		...
		public function wkwc_wallet_add_wallet_product_to_cart( $data ) {
			if ( ! empty( $data ) ) {
				...
				if ( empty( $add_money ) || ! is_numeric( $add_money ) ) {
					wc_print_notice( esc_html__( 'Enter a valid numeric amount to recharge your wallet.', 'wkwc_wallet' ), 'error' );
					return false;
				}

				if ( ! empty( $min_credit ) && $add_money < $min_credit ) {
					wc_print_notice( wp_sprintf( /* translators: %s: Minimum Credit amount. */ esc_html__( 'Minimum Wallet Credit Amount must be: %s', 'wkwc_wallet' ), wc_price( $min_credit ) ), 'error' );
					return false;
				}

				if ( ! empty( $max_credit ) && $add_money > $max_credit ) {
					wc_print_notice( wp_sprintf( /* translators: %s: Minimum Credit amount. */ esc_html__( 'Maximum Wallet Credit Amount limit is: %s', 'wkwc_wallet' ), wc_price( $max_credit ) ), 'error' );
					return false;
				}
				...
				if ( ! $is_valid_wallet ) {
					wc_print_notice( esc_html__( 'Wallet product is not valid.', 'wkwc_wallet' ), 'error' );
					return false;
				}
				...
				if ( $after_add > $max_amount ) {
					wc_print_notice( wp_sprintf( /* translators: %s: Minimum Credit amount. */ esc_html__( 'Maximum Wallet Amount could not be more than %s', 'wkwc_wallet' ), wc_price( $max_amount ) ), 'error' );
					return false;
				}
				...
				$added_text = wp_sprintf( /* translators: %s: Wallet product title. */ __( '%s has been added to your cart.', 'wkwc_wallet' ), strip_tags( get_the_title( $wallet_id ) ) );
				$message    = wp_sprintf( /* translators: %s: Cart URL, %s: Cart button, %s: Added text. */ '<a href="%s" tabindex="1" class="button wc-forward">%s</a> %s', esc_url( wc_get_cart_url() ), esc_html__( 'View cart', 'wkwc_wallet' ), esc_html( $added_text ) );
		...
		public function wkwc_wallet_update_cart_price( $cart_object ) {
			...
			if ( $wallet_money > 0 ) {
				...
				foreach ( $cart_items as $card_data ) {
					$cart_product_id = empty( $card_data['product_id'] ) ? 0 : intval( $card_data['product_id'] );

					if ( $cart_product_id === $wallet_prod_id ) {
				...
				if ( ! empty( $get_cart ) ) {
					...
					foreach ( $get_cart as $value ) {
						...
						if ( intval( $wallet_id ) === $cart_product_id ) {
							wc_add_notice( esc_html__( 'Cannot add new product now. Either empty cart or process it first.', 'wkwc_wallet' ) );
						}
					}
				}
			}
		...
		public function wkwc_wallet_front_footer_info() {
			...
			if ( 200 === $show_info ) {
				?>
			<input type="hidden" data-lwd="202312201115" data-wkwc_wallet_version="<?php echo esc_attr( WKWC_WALLET_SCRIPT_VERSION ); ?>">
				<?php
			}
		}
	}
