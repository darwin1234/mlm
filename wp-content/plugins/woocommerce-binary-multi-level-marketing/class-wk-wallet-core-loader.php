...
if ( ! class_exists( 'WKWP_Wallet_WK_Wallet_Core' ) ) {
	...
	class WKWP_Wallet_WK_Wallet_Core {
		...
		public static function load_files() {
			$get_global_path = BMLM_PLUGIN_FILE . 'wkwc_wallet/';

			if ( false === @file_exists( $get_global_path . '/includes/class-wkwc-wallet.php' ) ) {
				_doing_it_wrong( __FUNCTION__, esc_html__( 'WKWC Wallet should be present in folder \'wkwc_wallet\includes\' in order to run this properly.', 'binary-mlm' ), esc_html( self::$version ) );
			...
			if ( WKWC_WALLET_VERSION === self::$version ) {
				...
				_doing_it_wrong( __FUNCTION__, esc_html__( 'WK Wallet Core should be at the same version as declared in your "class-wk-wallet-core-loader.php"', 'binary-mlm' ), esc_html( self::$version ) );
				if ( defined( 'WKWC_DEV' ) && true === WKWC_DEV ) {
					die( 0 );
				}
				return false;
